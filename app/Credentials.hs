...
import Types (AccessToken, AppError (AppPrivateKeyReadError, AppServiceAccountDecodeError), Email (Email), Scope (unScope))
...
getAccessToken :: (MonadIO m) => SignedJWT -> m AccessToken
